...
public class Server {
    ...
    static ArrayList<ServerChompGame> ConnectedClients;

    public Server(int port) {

        Server.port = port;

        Server.ConnectedClients = new ArrayList<ServerChompGame>();
    }
    ...
    public void stop() {
        ...
        try {
            for (int i = ConnectedClients.size(); --i >= 0;) {
                ServerChompGame ct = ConnectedClients.get(i);
                ct.close();;
    ...
    public void run() {
        ...
        try {
            ...
            while (!Server.serverSocket.isClosed()) {
                ...
                ServerChompGame newClient = new ServerChompGame(socket);
            ...
            try {
                ...
                for (int i = 0; i < Server.ConnectedClients.size(); i++) {
                    ServerChompGame tc = Server.ConnectedClients.get(i);
                    tc.close();
